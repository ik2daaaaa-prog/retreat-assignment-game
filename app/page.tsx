...
import { createFortuneDeck, createSeatAssignments, createVehicle, driverSelectionComplete, passengerSeatIds, remainingParticipantIds, removeFortuneCard, resetGamePhase, roomCandidateIds, selectRandom, validateCapacity, type Assignment, type FortuneCardId, type Phase, type Vehicle } from "./lib/assignment";
...
export default function Home() {
  ...
  const [fortuneDeck, setFortuneDeck] = useState<FortuneCardId[]>(createFortuneDeck);
  const [protectedParticipant, setProtectedParticipant] = useState<string | null>(null);
    ...
    const timer = window.setTimeout(() => {
      ...
      if (saved) {
        try {
          ...
          setPhase(data.phase || "setup"); setParticipants(data.participants || []); setDriverIds(data.driverIds || []); setDriverTarget(data.driverTarget || 1); setVehicleLabels(data.vehicleLabels || {}); setVehicles(data.vehicles || []); setVehicleSeatAssignments(data.vehicleSeatAssignments || {}); setSeatIndex(data.seatIndex || 0); setRooms(data.rooms || [{ id: "room-1", label: "1호실", capacity: 2 }]); setRoomAssignments(data.roomAssignments || {}); setRoomSlotIndex(data.roomSlotIndex || 0); setFortuneDeck(data.fortuneDeck || createFortuneDeck()); setProtectedParticipant(data.protectedParticipant || null);
  ...
  useEffect(() => { if (hydrated) localStorage.setItem("retreat-assignment-game-v1", JSON.stringify({ phase, participants, driverIds, driverTarget, vehicleLabels, vehicles, vehicleSeatAssignments, seatIndex, rooms, roomAssignments, roomSlotIndex, fortuneDeck, protectedParticipant })); }, [hydrated, phase, participants, driverIds, driverTarget, vehicleLabels, vehicles, vehicleSeatAssignments, seatIndex, rooms, roomAssignments, roomSlotIndex, fortuneDeck, protectedParticipant]);

  const reset = () => { localStorage.removeItem("retreat-assignment-game-v1"); setPhase(resetGamePhase()); setParticipants([]); setDriverIds([]); setDriverTarget(1); setVehicleLabels({}); setVehicles([]); setVehicleSeatAssignments({}); setSeatIndex(0); setRooms([{ id: "room-1", label: "1호실", capacity: 2 }]); setRoomAssignments({}); setRoomSlotIndex(0); setCapacityError(""); setRoomError(""); setFortuneDeck(createFortuneDeck()); setProtectedParticipant(null); setCardMessage(""); setRouletteKey(0); };
  const useFortuneCard = (cardId: FortuneCardId) => {
    setFortuneDeck((current) => removeFortuneCard(current, cardId));
    if (cardId === "driverSwap") {
      const candidate = selectRandom(driverCandidates);
      if (candidate && driverIds.length) { setDriverIds((current) => [...current.slice(0, -1), candidate.id]); setCardMessage(`운전자 교체! ${candidate.label}님이 마지막 운전자를 대신합니다.`); }
      else setCardMessage("교체할 운전자 후보가 없습니다.");
    }
    if (cardId === "seatProtect") {
      const candidate = selectRandom(seatCandidates);
      setProtectedParticipant(candidate?.id || null); setCardMessage(candidate ? `${candidate.label}님이 다음 좌석을 보호받습니다.` : "보호할 좌석 후보가 없습니다.");
    }
    if (cardId === "reroll") { setRouletteKey((current) => current + 1); setCardMessage("한 번 더 돌립니다. 결과가 뒤집힐 수 있습니다!"); }
  };
  ...
  return <main className="app-shell">
    ...
    {phase === "setup" && <ParticipantSetup participants={participants} onChange={setParticipants} onStart={(count) => { setDriverTarget(count); setDriverIds([]); setFortuneDeck(createFortuneDeck()); setProtectedParticipant(null); setCardMessage(""); setPhase("drivers"); }} />}
    {phase === "drivers" && <section className="game-card"><div className="section-kicker">ROUND 01 · LADDER DRIVER DRAW</div><h2>사다리 끝에서 운전자가 결정됩니다</h2><p className="intro">필요한 운전자 수: <b>{driverTarget}명</b> · 현재 추첨: <b>{driverIds.length}/{driverTarget}명</b></p><div className="game-grid"><LadderBoard key={`driver-${rouletteKey}`} candidates={driverCandidates} onComplete={(winner) => setDriverIds((current) => current.length < driverTarget ? [...current, winner.id] : current)} buttonLabel="운전자 사다리 타기" /><aside className="result-panel"><div className="panel-label">확정된 운전자</div>{driverIds.length ? driverIds.map((id, index) => <div className="result-row" key={id}><span>DRIVER {String(index + 1).padStart(2, "0")}</span><strong>{id}</strong></div>) : <p className="empty-state">아직 사다리를 타지 않았습니다.</p>}<button className="primary-button" type="button" disabled={!driverSelectionComplete(driverIds.length, driverTarget)} onClick={() => setPhase("vehicles")}>운전자 확정 <span>→</span></button></aside></div><FortuneCards available={fortuneDeck} onUse={useFortuneCard} />{cardMessage && <p className="card-message" role="status">{cardMessage}</p>}<button className="back-button" type="button" onClick={() => setPhase("setup")}>← 참가자 수정</button></section>}
    ...
    {phase === "seats" && currentSeat && <section className="game-card"><div className="section-kicker">ROUND 03 · LADDER SEAT DRAW {seatIndex + 1}/{seatQueue.length}</div><h2>사다리 끝의 좌석에 앉습니다</h2><p className="intro">현재 좌석: <b>{vehicles.find((vehicle) => vehicle.id === currentSeat.vehicleId)?.label}</b> · <b>{seatNames[currentSeat.seatId] || currentSeat.seatId}</b>{protectedParticipant && <> · 보호 대상: <b>{protectedParticipant}</b></>}</p><div className="game-grid"><LadderBoard key={`seat-${currentSeat.vehicleId}-${currentSeat.seatId}-${rouletteKey}`} candidates={seatCandidates} durationMs={3000} onComplete={(winner) => { const finalWinner = protectedParticipant ? { id: protectedParticipant, label: protectedParticipant } : winner; setVehicleSeatAssignments((current) => ({ ...current, [`${currentSeat.vehicleId}:${currentSeat.seatId}`]: finalWinner.id })); setProtectedParticipant(null); if (seatIndex + 1 >= seatQueue.length) setPhase("roomSetup"); else setSeatIndex((current) => current + 1); }} buttonLabel="좌석 사다리 타기" /><AssignmentBoard vehicles={vehicles} assignments={vehicleSeatAssignments} /></div><FortuneCards available={fortuneDeck} onUse={useFortuneCard} />{cardMessage && <p className="card-message" role="status">{cardMessage}</p>}<button className="back-button" type="button" onClick={() => setPhase("vehicles")}>← 차량 정보 수정</button></section>}
    ...
    {phase === "rooms" && currentRoomSlot && <section className="game-card"><div className="section-kicker">ROUND 04 · LADDER ROOM DRAW {roomSlotIndex + 1}/{roomQueue.length}</div><h2>{rooms.find((room) => room.id === currentRoomSlot.roomId)?.label} 사다리 타기</h2><p className="intro">사다리 끝에 도착한 참가자가 이 방에 배정됩니다.</p><div className="game-grid"><LadderBoard candidates={roomCandidates} durationMs={3000} onComplete={(winner) => { setRoomAssignments((current) => ({ ...current, [`${currentRoomSlot.roomId}:${currentRoomSlot.index}`]: winner.id })); if (roomSlotIndex + 1 >= roomQueue.length) setPhase("summary"); else setRoomSlotIndex((current) => current + 1); }} buttonLabel="방 사다리 타기" /><aside className="result-panel"><div className="panel-label">현재 방 결과</div>{Object.entries(roomAssignments).filter(([key]) => key.startsWith(`${currentRoomSlot.roomId}:`)).map(([key, value]) => <div className="result-row" key={key}><span>ROOM SEAT</span><strong>{value}</strong></div>)}</aside></div></section>}
