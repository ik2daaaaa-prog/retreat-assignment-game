...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    {phase === "drivers" && <section className="game-card"><div className="section-kicker">ROUND 01 · PINBALL DRIVER DRAW</div><h2>핀볼에서 운전자를 뽑습니다</h2><p className="intro">필요한 운전자 수: <b>{driverTarget}명</b> · 현재 추첨: <b>{driverIds.length}/{driverTarget}명</b></p><div className="game-grid"><PinballArena candidates={driverCandidates} onComplete={(winner) => setDriverIds((current) => current.length < driverTarget ? [...current, winner.id] : current)} buttonLabel="운전자 핀볼 시작" /><aside className="result-panel"><div className="panel-label">확정된 운전자</div>{driverIds.length ? driverIds.map((id, index) => <div className="result-row" key={id}><span>DRIVER {String(index + 1).padStart(2, "0")}</span><strong>{id}</strong></div>) : <p className="empty-state">아직 핀볼을 시작하지 않았습니다.</p>}<button className="primary-button" type="button" disabled={!driverSelectionComplete(driverIds.length, driverTarget)} onClick={() => setPhase("vehicles")}>운전자 확정 <span>→</span></button></aside></div><button className="back-button" type="button" onClick={() => setPhase("setup")}>← 참가자 수정</button></section>}
    ...
    {phase === "seats" && currentSeat && <section className="game-card"><div className="section-kicker">ROUND 03 · PINBALL SEAT DRAW {seatIndex + 1}/{seatQueue.length}</div><h2>핀볼 결과로 좌석을 정합니다</h2><p className="intro">현재 좌석: <b>{vehicles.find((vehicle) => vehicle.id === currentSeat.vehicleId)?.label}</b> · <b>{seatNames[currentSeat.seatId] || currentSeat.seatId}</b></p><div className="game-grid"><PinballArena key={`seat-${currentSeat.vehicleId}-${currentSeat.seatId}`} candidates={seatCandidates} durationMs={4600} onComplete={(winner) => { setVehicleSeatAssignments((current) => ({ ...current, [`${currentSeat.vehicleId}:${currentSeat.seatId}`]: winner.id })); if (seatIndex + 1 >= seatQueue.length) setPhase("roomSetup"); else setSeatIndex((current) => current + 1); }} buttonLabel="좌석 핀볼 시작" /><AssignmentBoard vehicles={vehicles} assignments={vehicleSeatAssignments} /></div><button className="back-button" type="button" onClick={() => setPhase("vehicles")}>← 차량 정보 수정</button></section>}
