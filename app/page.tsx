...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    {phase === "drivers" && <section className="game-card"><div className="section-kicker">ROUND 01 · SURVIVAL DRIVER DRAW</div><h2>마지막까지 살아남은 사람이 운전자!</h2><p className="intro">필요한 운전자 수: <b>{driverTarget}명</b> · 현재 추첨: <b>{driverIds.length}/{driverTarget}명</b></p><div className="game-grid"><Roulette key={`driver-${rouletteKey}`} candidates={driverCandidates} mood="survival" countdownSeconds={5} onComplete={(winner) => setDriverIds((current) => current.length < driverTarget ? [...current, winner.id] : current)} buttonLabel="생존자 추첨" /><aside className="result-panel"><div className="panel-label">확정된 운전자</div>{driverIds.length ? driverIds.map((id, index) => <div className="result-row" key={id}><span>DRIVER {String(index + 1).padStart(2, "0")}</span><strong>{id}</strong></div>) : <p className="empty-state">아직 추첨 결과가 없습니다.</p>}<button className="primary-button" type="button" disabled={!driverSelectionComplete(driverIds.length, driverTarget)} onClick={() => setPhase("vehicles")}>운전자 확정 <span>→</span></button></aside></div><FortuneCards available={fortuneDeck} onUse={useFortuneCard} />{cardMessage && <p className="card-message" role="status">{cardMessage}</p>}<button className="back-button" type="button" onClick={() => setPhase("setup")}>← 참가자 수정</button></section>}
    ...
    {phase === "seats" && currentSeat && <section className="game-card"><div className="section-kicker">ROUND 03 · SEAT DRAW {seatIndex + 1}/{seatQueue.length}</div><h2>10초 안에 좌석 운명이 결정됩니다</h2><p className="intro">현재 좌석: <b>{vehicles.find((vehicle) => vehicle.id === currentSeat.vehicleId)?.label}</b> · <b>{seatNames[currentSeat.seatId] || currentSeat.seatId}</b>{protectedParticipant && <> · 보호 대상: <b>{protectedParticipant}</b></>}</p><div className="game-grid"><Roulette key={`seat-${currentSeat.vehicleId}-${currentSeat.seatId}-${rouletteKey}`} candidates={seatCandidates} countdownSeconds={10} onComplete={(winner) => { const finalWinner = protectedParticipant ? { id: protectedParticipant, label: protectedParticipant } : winner; setVehicleSeatAssignments((current) => ({ ...current, [`${currentSeat.vehicleId}:${currentSeat.seatId}`]: finalWinner.id })); setProtectedParticipant(null); if (seatIndex + 1 >= seatQueue.length) setPhase("roomSetup"); else setSeatIndex((current) => current + 1); }} buttonLabel="좌석 추첨" /><AssignmentBoard vehicles={vehicles} assignments={vehicleSeatAssignments} /></div><FortuneCards available={fortuneDeck} onUse={useFortuneCard} />{cardMessage && <p className="card-message" role="status">{cardMessage}</p>}<button className="back-button" type="button" onClick={() => setPhase("vehicles")}>← 차량 정보 수정</button></section>}
