...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    {phase === "seats" && currentSeat && <section className="game-card"><div className="section-kicker">ROUND 03 · PINBALL SEAT DRAW {seatIndex + 1}/{seatQueue.length}</div><h2>공이 들어간 슬롯에 앉습니다</h2><p className="intro">현재 좌석: <b>{vehicles.find((vehicle) => vehicle.id === currentSeat.vehicleId)?.label}</b> · <b>{seatNames[currentSeat.seatId] || currentSeat.seatId}</b>{protectedParticipant && <> · 보호 대상: <b>{protectedParticipant}</b></>}</p><div className="game-grid"><PinballBoard key={`seat-${currentSeat.vehicleId}-${currentSeat.seatId}-${rouletteKey}`} candidates={seatCandidates} durationMs={5000} onComplete={(winner) => { const finalWinner = protectedParticipant ? { id: protectedParticipant, label: protectedParticipant } : winner; setVehicleSeatAssignments((current) => ({ ...current, [`${currentSeat.vehicleId}:${currentSeat.seatId}`]: finalWinner.id })); setProtectedParticipant(null); if (seatIndex + 1 >= seatQueue.length) setPhase("roomSetup"); else setSeatIndex((current) => current + 1); }} buttonLabel="좌석 공 발사" /><AssignmentBoard vehicles={vehicles} assignments={vehicleSeatAssignments} /></div><FortuneCards available={fortuneDeck} onUse={useFortuneCard} />{cardMessage && <p className="card-message" role="status">{cardMessage}</p>}<button className="back-button" type="button" onClick={() => setPhase("vehicles")}>← 차량 정보 수정</button></section>}
    ...
    {phase === "rooms" && currentRoomSlot && <section className="game-card"><div className="section-kicker">ROUND 04 · PINBALL ROOM DRAW {roomSlotIndex + 1}/{roomQueue.length}</div><h2>{rooms.find((room) => room.id === currentRoomSlot.roomId)?.label} 슬롯을 맞혀보세요</h2><p className="intro">공이 멈춘 방 슬롯에 참가자가 배정됩니다.</p><div className="game-grid"><PinballBoard candidates={roomCandidates} durationMs={4000} onComplete={(winner) => { setRoomAssignments((current) => ({ ...current, [`${currentRoomSlot.roomId}:${currentRoomSlot.index}`]: winner.id })); if (roomSlotIndex + 1 >= roomQueue.length) setPhase("summary"); else setRoomSlotIndex((current) => current + 1); }} buttonLabel="방 공 발사" /><aside className="result-panel"><div className="panel-label">현재 방 결과</div>{Object.entries(roomAssignments).filter(([key]) => key.startsWith(`${currentRoomSlot.roomId}:`)).map(([key, value]) => <div className="result-row" key={key}><span>ROOM SEAT</span><strong>{value}</strong></div>)}</aside></div></section>}
