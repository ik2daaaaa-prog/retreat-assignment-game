...
export default function Home() {
  ...
  return <main className="app-shell">
    <header className="topbar"><button className="brand-home" type="button" onClick={reset} aria-label="처음 화면으로 돌아가기"><span className="brand-lockup"><span className="brand-mark">ㅋㅋㅋ</span><span><span className="section-kicker">WEEKEND RETREAT · RANDOMIZER</span><h1>응~<em>나만 아니면 돼~</em></h1></span></span></button><button className="text-button" type="button" onClick={reset}>전체 초기화</button></header>
    ...
    {phase === "drivers" && <section className="game-card"><div className="section-kicker">ROUND 01 · ROULETTE DRIVER DRAW</div><h2>룰렛으로 운전자를 뽑습니다</h2><p className="intro">필요한 운전자 수: <b>{driverTarget}명</b> · 현재 추첨: <b>{driverIds.length}/{driverTarget}명</b></p><div className="game-grid"><Roulette candidates={driverCandidates} countdownSeconds={3} mood="survival" onComplete={(winner) => setDriverIds((current) => current.length < driverTarget ? [...current, winner.id] : current)} buttonLabel="운전자 룰렛 시작" /><aside className="result-panel"><div className="panel-label">확정된 운전자</div>{driverIds.length ? driverIds.map((id, index) => <div className="result-row" key={id}><span>DRIVER {String(index + 1).padStart(2, "0")}</span><strong>{id}</strong></div>) : <p className="empty-state">아직 룰렛을 시작하지 않았습니다.</p>}<button className="primary-button" type="button" disabled={!driverSelectionComplete(driverIds.length, driverTarget)} onClick={() => setPhase("vehicles")}>운전자 확정 <span>→</span></button></aside></div><button className="back-button" type="button" onClick={() => setPhase("setup")}>← 참가자 수정</button></section>}
    ...
    {phase === "seats" && currentSeat && <section className="game-card"><div className="section-kicker">ROUND 03 · ROULETTE SEAT DRAW {seatIndex + 1}/{seatQueue.length}</div><h2>룰렛으로 좌석을 정합니다</h2><p className="intro">현재 좌석: <b>{vehicles.find((vehicle) => vehicle.id === currentSeat.vehicleId)?.label}</b> · <b>{seatNames[currentSeat.seatId] || currentSeat.seatId}</b></p><div className="game-grid"><Roulette key={`seat-${currentSeat.vehicleId}-${currentSeat.seatId}`} candidates={seatCandidates} countdownSeconds={5} onComplete={(winner) => { setVehicleSeatAssignments((current) => ({ ...current, [`${currentSeat.vehicleId}:${currentSeat.seatId}`]: winner.id })); if (seatIndex + 1 >= seatQueue.length) setPhase("roomSetup"); else setSeatIndex((current) => current + 1); }} buttonLabel="좌석 룰렛 시작" /><AssignmentBoard vehicles={vehicles} assignments={vehicleSeatAssignments} /></div><button className="back-button" type="button" onClick={() => setPhase("vehicles")}>← 차량 정보 수정</button></section>}
    ...
    {phase === "rooms" && currentRoomSlot && <section className="game-card"><div className="section-kicker">ROUND 04 · ROULETTE ROOM DRAW {roomSlotIndex + 1}/{roomQueue.length}</div><h2>{rooms.find((room) => room.id === currentRoomSlot.roomId)?.label} 룰렛</h2><p className="intro">룰렛 결과에 따라 참가자를 방에 배정합니다.</p><div className="game-grid"><Roulette candidates={roomCandidates} countdownSeconds={4} onComplete={(winner) => { setRoomAssignments((current) => ({ ...current, [`${currentRoomSlot.roomId}:${currentRoomSlot.index}`]: winner.id })); if (roomSlotIndex + 1 >= roomQueue.length) setPhase("summary"); else setRoomSlotIndex((current) => current + 1); }} buttonLabel="방 룰렛 시작" /><aside className="result-panel"><div className="panel-label">현재 방 결과</div>{Object.entries(roomAssignments).filter(([key]) => key.startsWith(`${currentRoomSlot.roomId}:`)).map(([key, value]) => <div className="result-row" key={key}><span>ROOM SEAT</span><strong>{value}</strong></div>)}</aside></div></section>}
